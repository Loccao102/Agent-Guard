...
func Open(path string) (*Store, error) {
	...
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT,timestamp TEXT NOT NULL,agent TEXT NOT NULL,kind TEXT NOT NULL,value TEXT NOT NULL,decision TEXT NOT NULL,risk TEXT NOT NULL,reason TEXT NOT NULL,rule_id TEXT NOT NULL DEFAULT '',previous_hash TEXT NOT NULL DEFAULT '',hash TEXT NOT NULL DEFAULT ''); CREATE INDEX IF NOT EXISTS idx_events_timestamp ON events(timestamp DESC);`)
	if err != nil {
		db.Close()
	...
	return &Store{db: db, redactor: redact.New()}, nil
}
...
func (s *Store) Record(e Event) error {
	...
	defer tx.Rollback()
	var previous string
	err = tx.QueryRow(`SELECT hash FROM events WHERE hash <> '' ORDER BY id DESC LIMIT 1`).Scan(&previous)
	if err != nil && err != sql.ErrNoRows {
		return err
	}
	e.PreviousHash = previous
	e.Hash = eventHash(e)
	_, err = tx.Exec(`INSERT INTO events(timestamp,agent,kind,value,decision,risk,reason,rule_id,previous_hash,hash) VALUES(?,?,?,?,?,?,?,?,?,?)`, e.Timestamp.Format(time.RFC3339Nano), e.Agent, e.Kind, e.Value, e.Decision, e.Risk, e.Reason, e.RuleID, e.PreviousHash, e.Hash)
	if err != nil {
		return err
	}
	return tx.Commit()
...
func eventHash(e Event) string {
	parts := []string{e.Timestamp.UTC().Format(time.RFC3339Nano), e.Agent, e.Kind, e.Value, e.Decision, e.Risk, e.Reason, e.RuleID, e.PreviousHash}
